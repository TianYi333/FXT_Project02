...
void SSD1309_DrawString(int x, int y, const char* str, int invert) {
    while (*str) {
        ...
        for (int col = 0; col < 8; col++) {
            uint8_t font_data = FONT_8x8[char_index * 8 + col];
...
// 垂直绘制函数（X轴向下，Y轴向右）
void SSD1309_DrawVerticalString(int start_col, int start_page, const char* str) {
    uint8_t *p = (uint8_t*)str;
    while (*p) {
        // 1. 计算字符索引
        uint8_t char_idx = (*p++) - 0x20;
        if (char_idx > 0x5E) char_idx = 0;

        // 2. 获取旋转后的字模数据
        const uint8_t* font = &FONT_8x8_ROT90[char_idx * 8];
        ...
        for (int col=0; col<8; col++) {
            ...
            for (int bit=0; bit<8; bit++) {
                if (data & (1 << bit)) {
                    SSD1309_DrawPixel(start_col + col, start_page*8 + bit, 1);
                }
            }
        }
        start_col += 8; // 向右移动字符宽度
    }
    SSD1309_Update();
}

void OLED_ShowChinese(uint8_t x, uint8_t y, const uint8_t *font) {
    for (uint8_t page=0; page<2; page++) { // 分两页显示（16行）
        OLED_SetCursor(y + page, x);        // 设置页地址
        for (uint8_t col=0; col<16; col++) {
            uint8_t data = font[page*16 + col];
            SSD1309_Write(1,data);          // 逐列发送数据
        }
    }
}

// 修正后的汉字显示函数
void OLED_ShowChinese_Fix(uint8_t x, uint8_t y, const uint8_t *font) {
    for (uint8_t col=0; col<16; col++) { // 遍历16列
        OLED_SetCursor(y, x + col);      // 设置到目标列
        // 发送当前列的上半部分（前16字节）
        SSD1309_Write(1,font[col]);
        // 发送当前列的下半部分（后16字节）
        SSD1309_Write(1,font[col + 16]);
    }
}
/**
 * @brief 设置显存操作起始地址
 * @param page 页地址（0-7，对应屏幕的8行一组）
 * @param column 列地址（0-127）
 */
void OLED_SetCursor(uint8_t page, uint8_t column) {
    // 确保地址在有效范围内
    page = page & 0x07;    // 限制页地址0-7
    column = column & 0x7F;// 限制列地址0-127

    // 发送页地址命令：0xB0 | page
    GPIO_Set(PIN_DC, 0);   // DC=0: 命令模式
    SPI_SendByte(0xB0 | page);

    // 发送列地址低4位：0x00 | (column & 0x0F)
    SPI_SendByte(0x00 | (column & 0x0F));

    // 发送列地址高4位：0x10 | ((column >> 4) & 0x0F)
    SPI_SendByte(0x10 | ((column >> 4) & 0x0F));

    GPIO_Set(PIN_DC, 1);   // DC=1: 恢复数据模式
}
